...
const handle = async () =>{
    ...
    try{
        ...
        const {error:stripeError} = await stripe.value.confirmPayment({
            ...
            confirmParams:{
                return_url: `${window.location.origin}/checkout/membership/success/${membership.value.id}`,
                payment_method_data:{
                    billing_details:{
                        name:name.value,
                        email:email.value,
...
<template>
    <PublicAppLayout>
        ...
        <div class="w-19/20 md:w-7/10 lg:w-1/2 border shadow-xl m-auto p-4 my-10 rounded-md">
            <p class="font-bold text-xl">Detalles de compra</p>
            <hr class="mb-2">
            
            <div class="space-y-2">
                <p>Membresía: <span class="font-bold text-indigo-700">{{ membership.name }}</span> </p>
                <hr>
                <p>Costo de entrada: <span class="font-bold text-indigo-700">${{ membership.price }}</span> </p>
            </div>
